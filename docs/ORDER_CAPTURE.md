...
```powershell
python -m flows.order_capture.capture_screen pinduoduo --device <device_serial>
```
...
```powershell
python flows/meituan_order_bot.py capture-scroll --device <device_serial> --pages 5 --start-x 540 --start-y 1700 --end-x 540 --end-y 500 --duration-ms 600
```

不知道总页数时，使用自动到底模式：

```powershell
python flows/pdd_order_bot.py capture-until-end --device <device_serial> --max-pages 50 --wait 1.5
python flows/meituan_order_bot.py capture-until-end --device <device_serial> --max-pages 50 --wait 1.5
```
...
```powershell
python flows/meituan_order_bot.py capture-until-end --device <device_serial> --max-pages 50 --stable-threshold 0.998 --start-y 1800 --end-y 450
```

如果希望恢复旧行为，检测到画面不变时直接停止：

```powershell
python flows/meituan_order_bot.py capture-until-end --device <device_serial> --max-pages 50 --stop-on-stable
```
