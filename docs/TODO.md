...
- [x] 统一 macOS/Windows USB 安卓设备发现、授权检查、截图、屏幕尺寸、坐标换算和滑动。
- [x] App 名称、输出目录和滑动参数迁移到本机 `common.env`。
- [x] 增加光大银行账单页面截图配置。
- [ ] 定义银行 App 截图流水 schema，至少覆盖交易时间、金额、方向、对方、摘要、账户和来源截图。
- [ ] 实现光大银行截图结构化识别与跨截图去重。
- [ ] 将安卓截图结构化流水接入 normalized 和 ledger 主链路。
- [ ] 把邮件/PDF 结果降级为 `reconciliation` 输入，生成与安卓流水的逐笔差异报告。
