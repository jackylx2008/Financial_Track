...
用途：
  读取流水邮件原始记录，汇总邮件附件信息并检查密码配置准备情况，为后续附件解密提取流程生成清单。
...
可选参数：
  ...
  --records       流水邮件记录 JSONL 路径，默认 `raw_data/financial_email/financial_email_records.jsonl`。
  --password-env  私有附件密码环境文件路径；未传入时使用 `config.yaml` 中的配置。
  --output-dir    附件清单输出目录，默认 `raw_data/financial_email`。
...
from flows.workflows.financial_attachment_prepare import run
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare financial email attachment inventory and password readiness.")
    ...
    parser.add_argument(
        "--output-dir",
        default="raw_data/financial_email",
