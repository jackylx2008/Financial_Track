...
用途：
  作为拼多多平台入口，默认选择拼多多平台并复用 Android 订单截图采集流程实现。

配置文件：
  本入口不读取 `config.yaml` 或 `.env`。ADB 路径优先使用 `--adb` 指定值，其次使用系统 PATH，
  再尝试常见模拟器自带 ADB。默认输出目录为 `raw_data/pdd`。
...
可选参数：
  ...
  --adb               adb.exe 路径。
  --output-dir        截图输出目录，默认 `raw_data/pdd`。
  --keep-existing     保留输出目录中已有的拼多多截图。
...
示例：
  ...
  python flows/pdd_order_bot.py capture-until-end --device <device_serial>
...
from flows.android_order.android_order_bot import main
