...
示例：
  python flows/financial_email_bot.py --since 2024-01-01
  python flows/financial_email_bot.py --eml-dir raw_data/financial_email/eml
...
from flows.workflows.financial_attachment_extract import run as run_attachment_extract
from flows.workflows.financial_attachment_prepare import run as run_attachment_prepare
...
def run_crack_stage(args: argparse.Namespace) -> dict[str, Any]:
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "flows" / "financial_attachment_crack.py"),
        "--config",
        args.config,
        "--inventory",
        args.inventory,
