...
用途：
  读取拼多多或美团 Android 订单截图，调用外部视觉模型服务识别可见订单信息，并生成结构化 JSON。

配置文件：
  默认读取项目根目录 `config.yaml`，其中 `app` 负责日志级别，`llamacpp` 负责外部视觉模型服务连接参数。
  `common.env` 可为服务地址、模型名和 API 密钥等环境变量占位提供覆盖值。
...
示例：
  python flows/order_image_ai.py pdd --all
  python flows/order_image_ai.py meituan --image raw_data/meituan/example.png
...
from flows.workflows.order_image_extract import run
