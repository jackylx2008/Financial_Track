...
底层处理集中在模块层；用户应通过 ``flows/financial_attachment_crack.py``
或 ``flows/financial_email_bot.py --stage crack`` 启动。
"""
...
def print_summary(
    ...
    if cracked_count:
        ...
        print(
            "下次运行会先尝试已保存密码；需要提取附件时运行：python flows/financial_email_bot.py --stage extract"
        )
