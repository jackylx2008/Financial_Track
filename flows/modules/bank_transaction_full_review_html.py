...
def _review_row(transaction: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_links": [_source_link(item) for item in source_records],
    }
...
def _source_link(source: dict[str, Any]) -> dict[str, str]:
    value = str(source.get("source_file") or source.get("body_text_file") or "")
    if not value:
        return {"label": "—", "path": "", "uri": ""}
    path = Path(value).expanduser()
    try:
        uri = path.resolve().as_uri()
    except ValueError:
        uri = ""
    return {"label": _source_location(source), "path": str(path.resolve()), "uri": uri}
...
<div class="table-wrap"><table><thead><tr><th style="width:45px">#</th><th style="width:145px">机构/卡片类型</th><th style="width:75px">卡号尾号</th><th style="width:145px">交易日期</th><th style="width:95px">入账日期</th><th style="width:65px">方向</th><th style="width:105px">交易金额</th><th style="width:70px">币种</th><th style="width:220px">商户/对方全名</th><th style="width:180px">对方账号</th><th style="width:110px">交易类型</th><th style="width:235px">摘要</th><th style="width:105px">渠道</th><th style="width:250px">来源定位/其他</th></tr></thead><tbody id="body"></tbody></table><div id="empty" class="empty" hidden>没有符合筛选条件的交易。</div></div>
