...
<section class="filtered-summary"><h2>当前筛选金额汇总</h2><p>按币种分别统计当前筛选结果；总金额为收入与支出金额之和。</p><table class="totals-table"><thead><tr><th>币种</th><th>笔数</th><th>总金额</th><th>收入金额</th><th>支出金额</th></tr></thead><tbody id="summaryBody"></tbody></table></section>
...
function addOptions(id,values){values.forEach(value=>{const option=document.createElement('option');option.value=value;option.textContent=value;$(id).append(option)})}addOptions('institutionFilter',data.institutions);addOptions('cardRoleFilter',data.card_roles);addOptions('currencyFilter',data.currencies);addOptions('sourceFilter',data.sources);
...
function formatAmount(cents){return(cents/100).toLocaleString('zh-CN',{minimumFractionDigits:2,maximumFractionDigits:2})}function renderSummary(){const totals=new Map();filtered.forEach(row=>{if(row.amount_value===null||!Number.isFinite(row.amount_value))return;const currency=row.currency||'—',entry=totals.get(currency)||{count:0,total:0,inflow:0,outflow:0},cents=Math.round(Math.abs(row.amount_value)*100);entry.count+=1;entry.total+=cents;if(row.direction==='inflow')entry.inflow+=cents;if(row.direction==='outflow')entry.outflow+=cents;totals.set(currency,entry)});const body=$('summaryBody');body.replaceChildren();if(!totals.size){const tr=node('tr'),td=node('td','当前筛选结果没有可汇总金额');td.colSpan=5;tr.append(td);body.append(tr);return}[...totals.entries()].sort(([a],[b])=>a.localeCompare(b,'zh-CN')).forEach(([currency,entry])=>{const tr=node('tr');appendCell(tr,currency);appendCell(tr,entry.count.toLocaleString());appendCell(tr,formatAmount(entry.total),'money');appendCell(tr,formatAmount(entry.inflow),'money');appendCell(tr,formatAmount(entry.outflow),'money');body.append(tr)})}
