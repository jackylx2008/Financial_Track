...
def _review_row(order: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "flow_hash_sha256": str(order.get("flow_hash_sha256") or ""),
        "record_fingerprint_sha256": str(order.get("record_fingerprint_sha256") or ""),
        ...
        "sources": _all_source_links(sources),
        "source_record_hashes": [
            str(source.get("source_record_sha256")) for source in sources if source.get("source_record_sha256")
        ],
    }


def _all_source_links(sources: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    for source in sources:
        for field, kind in fields:
            ...
            if key in seen or not path.is_file():
                continue
            seen.add(key)
            links.append(_source_link(path, kind, source))
    return links
