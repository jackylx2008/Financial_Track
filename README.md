...
- 对归一化银行流水执行大额资金池追溯，并通过可交互的节点与连线图审核上游来源和后续去向。
...
`large_fund_trace_review.html`；页面默认按金额从大到小排列大额转入、转出。主清单采用资金流卡片而不是
数据表格；点击流水后弹出节点与连线组成的资金链路图，点击转出查看推断的上游资金来源，点击转入查看已经
分配到的后续去向。链路节点可继续点击查看交易说明和原始来源定位。当前模型在同一银行账户、同一币种内建立资金池，所有
...
独立重建只读取 `bank_transactions.jsonl`，不会重新扫描 PDF，也不会调用本地 AI/OCR。生成后的 HTML 仍应
从 GUI 打开，这样节点详情中的原始 PDF、Excel、邮件等来源链接才能通过本机安全桥接交给默认程序。
